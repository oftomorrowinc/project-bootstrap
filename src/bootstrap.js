...
program
  ...
  .action(async (templateName, options) => {
      ...
      templates.forEach((template) => {
        try {
          const templateConfig = yaml.load(
            fs.readFileSync(path.join(templatesDir, template, 'projectfile.yml'), 'utf8')
          );
    ...
    console.log(
      chalk.green(
        `Template '${templateName}' initialized! Copied projectfile.yml to current directory.`
      )
    );
...
async function bootstrapProject(projectFilePath) {
  ...
  if (config.files) {
    for (const [filename, destPath] of Object.entries(config.files)) {
      if (typeof destPath === 'string') {
        ...
        console.error(
          chalk.red(
            `Error: Invalid destination for file ${filename}. Expected string, got ${typeof destPath}`
          )
        );
      }
    }
  ...
  if (config.commands) {
    for (const cmd of config.commands) {
      ...
      if (
        cmd.includes('{{devDependencies}}') &&
        config.packages &&
        config.packages.devDependencies
      ) {
        processedCmd = cmd.replace(
          '{{devDependencies}}',
          config.packages.devDependencies.join(' ')
        );
      }
